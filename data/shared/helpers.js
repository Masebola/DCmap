...
export function issue(series, volumeYear, issueNumber, releaseYear = volumeYear, options = {}) {
  ...
  return {
    id: options.id || `${slug(series)}-${volumeYear}-${normaliseIssue(issueLabel)}`,
    series,
    volumeYear,
    issue: issueLabel,
    releaseYear,
...
export function range(series, volumeYear, start, end, releaseYear = volumeYear) {
  return Array.from({ length: end - start + 1 }, (_, index) => issue(series, volumeYear, start + index, releaseYear));
}
...
export function special(title, releaseYear, issueNumber = 1, options = {}) {
  return issue(title, releaseYear, issueNumber, releaseYear, { ...options, kind: 'special' });
}
